 ...
 %        local_foci_mask = object_separation(local_foci_mask,1)>0; % Get rid of small object and separate some clusters
    ...
    %temp_holder = newim(size(wave_foci_mask));
       ...
       % cor_spot = gaussf(local_spot,1)-gaussf(local_spot,6);
        ...
        local_nuc = local_nuc*(local_nuc==nuc_num);                     %isolate only the nuc_num id in local_nuc
        cor_spot = local_spot-gaussf(local_spot,10);              % Imput Seb
        %cor_spot = cor_spot* (cor_spot>2000);                    % Imput Seb
        cor_spot =  cor_spot* threshold(cor_spot,'isodata');
               ...
               %local_foci_mask = and (or(local_foci_mask_r2, local_foci_mask_r3),local_nuc>0);
    ...
    %temp_holder = newim(size(wave_foci_mask));
        ...
        if sum(and(local_foci_mask>0,local_nuc>0))    %if the local foci mask is not empty
            ...
            try
                ...
                local_spot_single = watershedminima (-local_spot, 2, max_int, max_size, 'binary');
                %local_spot_single = dip_localminima(-(add_z_ends(local_spot)), add_z_ends(local_foci_mask)>0, 3, max_int, max_size, 1);   %initial foci seed detection
                %local_spot_single = local_spot_single(:,:,1:end-1);
                local_foci_mask = growregions(label(local_spot_single),local_foci_mask, 2, 0);
                local_foci_mask = areaopening (local_foci_mask,2); % Discard single pixels
                % Weed out dim foci (mean Int < min_int)
                % S. Costes, September 2009
                % trying with mean Int < min_int + 0.1(max_int-min_int)
                local_foci_mask = areaopening(local_foci_mask,10);             % Imput Seb
        ...
        %if ~isempty(large_seeds)
         %  for m = large_seeds
          %      new_large_seed_coords = round(local_label_spot_single_ms(m).center);
           %     local_label_spot_single = local_label_spot_single .* (local_label_spot_single ~= m);
            %    local_label_spot_single(new_large_seed_coords(1), new_large_seed_coords(2)) = m;
        ...
        %make it a binary
        local_foci_mask = dip_image(local_foci_mask > 0);

        %grow regions back
        %local_label_foci_mask = dip_growregions(local_label_spot_single, [], local_foci_mask, 3, 0, 'high_first', 'default');      
        local_label_foci_mask = growregions(local_label_spot_single, local_foci_mask, 2, 0);
        ...
        if (max(local_label_foci_mask) == 0)
           ...
           %info_spot = [info_spot; ones(num_spot,1)*nuc_num, spot_ID', spot_size', spot_mean', spot_max'];
            info_spot = [info_spot; ones(num_spot,1)*nuc_num, spot_ID(:), spot_size(:), spot_mean(:), spot_max(:)]; % Proposition ChatGPT
